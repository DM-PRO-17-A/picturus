...
class Daughter_Card:
    ...
    gtsrb_subset = ['20 Km/h', '30 Km/h', '50 Km/h', '60 Km/h', '70 Km/h', '80 Km/h',
                     'End 80 Km/h', '100 Km/h', '120 Km/h', 'Stop', 'Turn left ahead', 'Turn right ahead']
    
    actions = ("drive", "increase speed", "perform action")
    effort = {"drive": 0.5, "increase speed": 1, "perform action": 3}
    state = actions[0]
        
    def send(self, result, filename):
        f = open(filename, 'a')
        for i in range(len(result)):
            if int(result[i]*100) != 0:
                sign = self.gtsrb_classes[i]
                f.write('\t\tQNN predicts ' + sign + " with probability "+str(int(result[i]*100))+'%' '\n')
        f.write('\n')
        f.close()
